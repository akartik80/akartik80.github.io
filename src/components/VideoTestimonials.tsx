...
const VideoTestimonials = () => {
  ...
  return <section id="video-testimonials" className="py-24 relative overflow-hidden bg-gradient-to-b from-white via-autthia-purple/5 to-white">
      {/* Decorative elements */}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16 reveal-on-scroll">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">
              <span className="bg-gradient-to-r from-[#678bd4] via-[#678bd4]/80 to-[#678bd4]/60 bg-clip-text text-transparent font-semibold text-5xl">What Founders Are Saying</span>
            </h2>
          ...
          <div className="relative">
              ...
              <CardContent className="p-0">
                <div className="grid md:grid-cols-2 gap-0">
                  ...
                  <div className="relative aspect-video bg-black">
                    <img src={currentTestimonial.thumbnail} alt="Video thumbnail" className="w-full h-full object-cover opacity-80" />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Button variant="ghost" size="icon" className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/30 transition-all" onClick={() => setIsPlaying(!isPlaying)}>
            ...
            <div className="flex justify-between items-center mt-8">
              <Button variant="outline" size="icon" onClick={prevTestimonial} className="w-12 h-12 rounded-full border-2 border-autthia-purple hover:bg-autthia-purple hover:text-white transition-all">
                <ChevronLeft className="h-5 w-5" />
              </Button>

              {/* Pagination dots */}
              <div className="flex gap-2">
                {videoTestimonials.map((_, index) => <button key={index} onClick={() => {
                setCurrentIndex(index);
                setIsPlaying(false);
              }} className={`w-3 h-3 rounded-full transition-all ${index === currentIndex ? 'bg-autthia-purple' : 'bg-autthia-purple/30 hover:bg-autthia-purple/50'}`} />)}
              </div>

              <Button variant="outline" size="icon" onClick={nextTestimonial} className="w-12 h-12 rounded-full border-2 border-autthia-purple hover:bg-autthia-purple hover:text-white transition-all">
